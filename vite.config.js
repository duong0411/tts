...
// URL Public từ R2 của bạn (dựa trên ảnh bạn cung cấp)
    ...
    vue(),
    {
      ...
      configureServer(server) {
        server.middlewares.use('/api', async (req, res, next) => {
          ...
          // 1. API Liệt kê danh sách model (Lấy từ R2 hoặc local json)
          if (url === '/models' || url === '/models/') {
            // Ở local, ta quét các file .json để biết có những model nào
            const modelsDir = path.join(__dirname, 'public', 'tts-model', 'vi');
            let models = [];
            if (fs.existsSync(modelsDir)) {
          ...
          // 2. API Lấy file model (Redirect sang R2)
          const modelMatch = url.match(/^\/model\/(.+)$/);
          if (modelMatch) {
            const fileName = decodeURIComponent(modelMatch[1]);
            // Chuyển hướng mọi yêu cầu file model sang link R2 trực tiếp
